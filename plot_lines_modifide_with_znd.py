...
def main(**kwargs):
    ...
    with open(f"../../find_A/profiles_new/detonation.{profile_name}.dat", "r") as file:
        for line in file:
...
############################################################################
    ...
    T_list = []
    rho_list = []
    for M_cj in np.arange(1, 1.5*M, 0.1):
        T_cj=(((2*gamma*(M_cj**2)+1-gamma)*((M_cj**2)*(gamma-1)+2))/(((gamma+1)**2)*(M_cj**2)))
        P_cj=(1 + (2 * gamma / (gamma + 1)) * (M_cj**2 - 1))
        rho_cj=P_cj/T_cj

        P_list.append(P_cj)
        T_list.append(T_cj)
        rho_list.append(rho_cj)
...
############################################################################
    ...
    plt.plot(x_val_to_plot_for_M_cj, y_val_to_plot_for_M_cj, linestyle='--', linewidth=1, color='red')
